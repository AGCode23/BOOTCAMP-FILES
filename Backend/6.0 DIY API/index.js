...
app.post("/jokes", (req, res) => {
  const textJoke = req.body.text;
  const typeJoke = req.body.type;
  var lastID = jokes[jokes.length - 1].id + 1;
  const jokeContent = { id: lastID, jokeText: textJoke, jokeType: typeJoke };
...
app.put("/jokes/:id", (req, res) => {
  ...
  jokes = jokes.map((joke) => (joke.id === id ? updateJoke : joke));
  res.json(jokes[id - 1]);
});
...
app.patch("/jokes/:id", (req, res) => {
  ...
  jokes = jokes.map((joke) => (joke.id === id ? patchJoke : joke));
  res.json(jokes[id - 1]);
});
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (index > -1) {
    ...
    res
      .status(404)
      .json({ error: "Error! No joke with an ID of " + id + " has found." });
  }
});

//8. DELETE All jokes
app.delete("/all?", (req, res) => {
  const yourKey = req.query.key;
